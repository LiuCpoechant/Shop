...
namespace Shop.DAL
{
	...
	public partial class UserDao
	{
        ...
        public Shop.Model.User GetByname(int uname)
        {
            ...
            SqlParameter[] parameters = {
					new SqlParameter("@uname", SqlDbType.Int,10)
			};
        ...
        ///获取记录总数
        public int GetUnameNickNameRecordCount(string strWhere)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) from t_user where unickname like '%niname%' or uname like '%niname%'");
        ...
        ///条件分页获取数据列表
        public DataSet GetListNicknameUnameByPage(string strWhere, string orderby, int startIndex, int endIndex)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            strSql.Append(")AS Row, unickname,uname as niname from t_user where unickname like '%niname%' or uname like '%niname%'");
