...
namespace Shop.DAL
{
	...
	public partial class GoodsDao
	{
        ...
        public bool UpdateSmall(Shop.Model.Goods model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update t_goods set ");
            strSql.Append("gname=@gname,");
            strSql.Append("smallid=@smallid,");
        ...
        ///三表查询结果转实体
        public Shop.Model.Goods DataRowToGoodsModel(DataRow row)
        {
            Shop.Model.Goods model = new Shop.Model.Goods();
            if (row != null)
            {
        ...
        //三表查询
        public int GetBigSmallRecordCount(string strWhere)
        {
           StringBuilder strSql = new StringBuilder();
           strSql.Append("select count(*) from t_smallClass,t_bigClass,t_goods where t_goods.smallid=t_smallClass.smallid and t_smallClass.bigid=t_bigClass.bigid");
        ...
        public DataSet GetGoodsListByPage(string strWhere, string orderby, int startIndex, int endIndex)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            {
                strSql.Append("order by t_goods" + orderby);
            }
            else
            {
                strSql.Append("order by t_goods.gid asc");
            }
            strSql.Append(")AS Row, t_goods.gid, t_goods.goodsphoto, t_goods.gname, t_goods.descript, t_goods.goodsprice, t_goods.specialprice, t_goods.hot, t_goods.smallname, t_goods.bigname, t_goods.stock, t_goods.gview "+
                " from t_goods, t_bigClass, t_smallClass where t_goods.smallid=t_smallClass.smallid and t_smallClass.bigid=t_bigClass.bigid ");
        ...
        ///获取smallcount记录总数
        //public int GetSmallRecordCount(string strWhere)
        //{
        //    StringBuilder strSql = new StringBuilder();
        //    strSql.Append("select count(*) from t_goods where smallid=SmallId");
        //    if (strWhere.Trim() != "")
        //    {
        //        strSql.Append(strWhere);
        //    }
        ...
        /// 
        ///顶部查询的记录数
        public int GetTopRecordCount(string strWhere)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(1) FROM t_goods where gname like '%sgname%'");
            if (strWhere.Trim() != "")
            {
                strSql.Append(strWhere);
            }
