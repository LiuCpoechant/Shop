...
namespace Shop.BLL
{
	...
	{
		private readonly Shop.DAL.VoteDao dal=new Shop.DAL.VoteDao();
		public VoteService()
		{}
		#region  BasicMethod
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Shop.Model.Vote GetModelByCache(int vid)
		{
			
			string CacheKey = "VoteModel-" + vid;
			...
			return (Shop.Model.Vote)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Shop.Model.Vote> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Shop.Model.Vote> DataTableToList(DataTable dt)
		{
			List<Shop.Model.Vote> modelList = new List<Shop.Model.Vote>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Shop.Model.Vote model;
